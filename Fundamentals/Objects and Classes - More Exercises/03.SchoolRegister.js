   ...
   for(let line of input){
    ...
    let score = Number(tokens[2].split(': ')[1]);
      ...
      if(score > 3){
        ...
        schoolRegister[grade].push(students)
      
      }
      
   }

   let sordtedGrades = Object.keys(schoolRegister).sort((a,b) => a - b );

   for(let grade of sordtedGrades){
    let students = schoolRegister[grade];
    console.log(`${grade} Grade`);
    console.log(`List of students: ${students.map(s=> s.name).join(', ')}`)
    let average = students.map(x=> x.score).reduce((a,b)=> a + b,0) / students.length
